...
set echo off feedback off pagesize 60 linesize 2000 newpage 1 recsep off;
...
spool /home/ora9i/jerel/top_sql_gets.txt
...
select aa
  from ( select /*+ ordered use_nl (b st) */
          decode( st.piece
                , 0
                , '^^'||chr(10)||'&1|&2|&3|'||'&V_ESANP_TIME'||'|'||lpad(to_char((e.buffer_gets - nvl(b.buffer_gets,0))
                               ,'99,999,999,999')
                      ,15)||'|'||
